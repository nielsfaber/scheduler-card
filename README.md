...
| `default_repeat_type`    | string         | `repeat`                    | Default `repeat_type` for newly created schedules. Choose from `repeat`, `pause`, or `single`. This only affects the initial value when creating a schedule; it can still be changed in schedule options later.                                                                                           |
