...
import { Condition, EConditionMatchType, CardConfig, EntityElement, StatesConfig } from '../types';
...
export class ConditionEntityRow extends LitElement {
  ...
  firstUpdated() {
    ...
    this.entity = parseEntity(this.item.entity, this.hass, this.config);
    this.states = computeEntityStates(this.item.entity, this.hass, this.config);
  }
  ...
  getMatchTypeButton() {
    ...
    let value;
    const matchType = this.item.match_type;
    if (matchType == EConditionMatchType.Equal) value = localize('ui.panel.conditions.equal_to', this.hass.language);
    else if (matchType == EConditionMatchType.Unequal)
      value = localize('ui.panel.conditions.equal_to', this.hass.language);
    else if (matchType == EConditionMatchType.Below)
      value = this.hass.localize('ui.panel.config.automation.editor.conditions.numeric_state.below');
    else if (matchType == EConditionMatchType.Above)
      value = this.hass.localize('ui.panel.config.automation.editor.conditions.numeric_state.above');
  ...
  matchTypeButtonClick() {
    ...
    this.item = Object.assign({ ...this.item }, { match_type: value } as Condition);
  ...
  stateButtonClick() {
    if (!this.item || !this.entity || !this.states) return;
    this.selected = false;
    const state = this.item.state;
    if (!this.states || !Array.isArray(this.states)) return;
    let i = 0;
    for (i = 0; i < this.states.length; i++) {
      if (this.states[i] == state) break;
    }
    const value = i < this.states.length - 1 ? this.states[i + 1] : this.states[0];
    this.item = Object.assign({ ...this.item }, { state: value });
    this.fireEvent();
  }

  stateDecrement(time: number | null = null) {
    if (!this.item || !this.states || Array.isArray(this.states)) return;
    clearTimeout(this.timer);
    let state = Number(this.item.state);
    const cfg = this.states;
  ...
  getState() {
    if (!this.item || !this.states) return;
    const state = this.item.state;
    const cfg = this.states;
  ...
  fireEvent() {
    ...
    const myEvent = new CustomEvent('change', { detail: data });
