...
export function GetActionConfig(cfg: IActionConfig, entity: IHassEntity) {
  ...
  if (getDomainFromEntityId(obj.service) == domain)
    obj = <IActionConfig>extend(obj, { service: removeDomainFromEntityId(obj.service) });
...
function extendActionVariables(cfg: IActionConfig, entity: IHassEntity) {
  ...
  if ('options' in variableCfg) {
    ...
    let hasValueTemplates = Object.values(optionCfg)
    ...
    if (hasValueTemplates) {
      let discoveredOptions: IListVariableOption[] = entity.attributes[hasValueTemplates![1]]
        .filter((e) => Object.values(optionCfg).every((el) => el.value != e))
        .map((e) => <IListVariableOption>{ value: e });
  ...
  let match = actionConfig.find((cfg) => {
    ...
    if (cfg.variable && action.service_data) {
      if (Object.keys(action.service_data).includes(cfg.variable.field))
        return (
          action_id ==
          getActionId(<IHassAction>extend(action, { service_data: { [cfg.variable.field]: null } }, { compact: true }))
        );
    }
    return false;
  });
