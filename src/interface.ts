...
export function importEntry(entryString: string) {
  ...
  let res = EntryPattern.exec(entryString)!;
  ...
  if (res[1]) {
    ...
    dayList.sort();
    if (!(dayList.length == 1 && dayList[0] == 0)) Object.assign(entry, { days: { type: EDayType.Custom, custom_days: dayList.filter(e => e != 0) } });
  }
  ...
  if (res[4]) {
    //end time is defined
    let isSunTime = SunTimePattern.exec(res[4]);
