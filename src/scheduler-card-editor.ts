...
import { CardConfig, EditorMode, TRepeatType } from './types';
  ...
  render() {
    ...
    return html`
      <div class="card-config">
        ...
        <span>${localize('ui.panel.card_editor.fields.default_repeat_type.heading', this.hass)}</span>
        <div class="two-columns">
          <div class="column">
            <ha-formfield label="${hassLocalize('ui.components.calendar.event.repeat.label', this.hass)}">
              <ha-radio
                name="default_repeat_type"
                value="${TRepeatType.Repeat}"
                @change=${() => {
                  this._updateConfig({ default_repeat_type: TRepeatType.Repeat });
                }}
                ?checked=${(this._config.default_repeat_type || TRepeatType.Repeat) == TRepeatType.Repeat}
              >
              </ha-radio>
            </ha-formfield>
          </div>
          <div class="column">
            <ha-formfield label="${hassLocalize('ui.dialogs.more_info_control.vacuum.stop', this.hass)}">
              <ha-radio
                name="default_repeat_type"
                value="${TRepeatType.Pause}"
                @change=${() => {
                  this._updateConfig({ default_repeat_type: TRepeatType.Pause });
                }}
                ?checked=${this._config.default_repeat_type == TRepeatType.Pause}
              >
              </ha-radio>
            </ha-formfield>
          </div>
          <div class="column">
            <ha-formfield label="${hassLocalize('ui.common.delete', this.hass)}">
              <ha-radio
                name="default_repeat_type"
                value="${TRepeatType.Single}"
                @change=${() => {
                  this._updateConfig({ default_repeat_type: TRepeatType.Single });
                }}
                ?checked=${this._config.default_repeat_type == TRepeatType.Single}
              >
              </ha-radio>
            </ha-formfield>
          </div>
          ...
          <div style="display: flex; flex: 1; flex-direction: column">
            ...
            <ha-dropdown
              ...
              @wa-after-show=${(ev: Event) => {
                ((ev.target as HTMLElement).querySelector('ha-input') as HTMLInputElement).focus();
              }}
