...
export const humidifierModes = (
  ...
  const supportedModes: string[] =
    stateObj && Array.isArray(stateObj.attributes.available_modes) ? stateObj.attributes.available_modes : [];
  return filterCapabilities ? modeList.filter(e => supportedModes.find(m => m === e.value)) : modeList;
};

export const humidifierActions = (
  hass: HomeAssistant,
  stateObj: HassEntity | undefined,
  filterCapabilities: boolean
) => {
    ...
    },
    {
      ...
      variables: {
        humidity: levelVariable({
          ...
          min: stateObj?.attributes.min_humidity !== undefined ? stateObj?.attributes.min_humidity : 0,
          max: stateObj?.attributes.max_humidity !== undefined ? stateObj?.attributes.max_humidity : 100,
          step: 1,
          unit: '%',
        }),
      },
