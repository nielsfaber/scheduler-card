...
export const moveTimeslot = (
  ...
  for (let i = slotIdx - 1; i >= 0; i--) {
    ...
    if (slots[i].actions.length) {
      lowerLimit = i == slotIdx - 1 ? addTimeOffset(startTime(slots[i]), { minutes: 1 }) : stopTime(slots[i]);
      break;
    }
  ...
  if (!isDefined(slots[slotIdx].stop)) {
    for (let i = slotIdx + 1; i < slots.length; i++) {
      ...
      if (slots[i].actions.length) {
        upperLimit = i == slotIdx + 1 ? addTimeOffset(stopTime(slots[i]), { minutes: -1 }) : startTime(slots[i]);
        break;
      }
  ...
  if (computeDuration(oldTime, newTime, hass) <= 0) {
    //time is moved backwards or kept the same
    for (let i = slotIdx - 1; i >= 0; i--) {
      //walk through all slots prior to modified one
  ...
  if (computeDuration(oldTime, newTime, hass) >= 0) {
    //time is moved forward or kept the same
