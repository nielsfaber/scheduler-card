...
export function computeCommonActions(actionLists: Action[][]) {
  ...
  commonActions = commonActions.map(action => {
    const mergedVariables: VariableDictionary = Object.entries(action.variables || {}).map(([field, variable]) => {
    ...
    if (Object.values(mergedVariables).find(e => e.type == EVariableType.List && !(e as ListVariable).options.length)) return undefined;
