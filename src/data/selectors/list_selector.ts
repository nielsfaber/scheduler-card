...
export const parseListSelectorOption = (input: Record<string, any>) => {
  ...
  if (Object.keys(input).includes('name'))
    cfg = { ...cfg, label: String(input.name) };
  else if (Object.keys(input).includes('label'))
    cfg = { ...cfg, label: String(input.label) };
  else if (Object.keys(input).includes('value'))
    cfg = { ...cfg, label: String(input.value) };

  if (Object.keys(input).includes('value'))
    cfg = { ...cfg, value: String(input.value) };
  else if (Object.keys(input).includes('name'))
    cfg = { ...cfg, value: String(input.name) };
  else if (Object.keys(input).includes('label'))
