...
export class SchedulerCard extends LitElement {
  ...
  private _addClick(_ev: Event) {
    ...
    const defaultRepeatType = this._config.default_repeat_type;
    if (defaultRepeatType && Object.values(TRepeatType).includes(defaultRepeatType)) {
      clonedConfig = { ...clonedConfig, repeat_type: defaultRepeatType };
    }
    const params: SchedulerDialogParams = {
