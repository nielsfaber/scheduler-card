...
export class SchedulerCard extends LitElement {
  ...
  protected render(): TemplateResult {
    if (!this.newItem && !this.editItem) {
      ...
      return html`
        <ha-card>
          ${this.getTitle()} ${this.showRoutineEditor()}
        </ha-card>
      `;
    } else {
      return html`
        <ha-card>
          ${this.getTitle()} ${this.showEditor()}
        </ha-card>
      `;
    }
  }
          ...
          >
            ...
            <div class="list-item-switch">
              ${scheduleItem.enabled
                ? html`
                    <ha-switch checked="checked" @click="${e => this.toggleDisable(scheduleItem.id, e)}"></ha-switch>
                  `
                : html`
                    <ha-switch @click="${e => this.toggleDisable(scheduleItem.id, e)}"></ha-switch>
                  `}
            </div>
          </div>
        `;
  ...
  getGroups(): TemplateResult[] {
    ...
    if (!groups.length)
      return [
        html`
          <div class="text-field">${localize('instructions.no_groups_defined')}</div>
        `,
      ];
    return groups.map((el: IGroupElement) => {
      return html`
        ...
        >
          ${el.icon
            ? html`
                <ha-icon icon="${PrettyPrintIcon(el.icon)}" class="padded-right"></ha-icon>
              `
            : ''}
  ...
  getEntities(): TemplateResult[] {
    if (!this._selectedGroup)
      return [
        html`
          <div class="text-field">${localize('instructions.no_group_selected')}</div>
        `,
      ];
    ...
    if (!entities.length)
      return [
        html`
          <div class="text-field">${localize('instructions.no_entities_for_group')}</div>
        `,
      ];
  ...
  getActions() {
    ...
    let options_list = actions
      .filter(e => {
        return e.id != RoutineAction.id;
      })
      .map((el: IActionElement) => {
        return html`
          <mwc-button
  ...
  showRoutineEditor(): TemplateResult {
    ...
    let activeAction = activeSlot
      ? actions.find(e => {
          return e.id == activeSlot!.action;
        })
      : null;
    ...
    return html`
      ...
      ${activeAction && activeAction.variable
        ? this.getLevelPanel(activeAction.variable as ILevelVariableConfig, true)
        : ''}
  ...
  getPlannerActions() {
    if (this._slotNum === null)
      return html`
        <div class="text-field">select a timeslot first</div>
      `;
    let actions = this.Config.GetActionsForEntity(this._entry.entity);
    ...
    return actions
      ...
      .map((el: IActionElement) => {
        return html`
          <mwc-button
            class="${activeSlot.action == el.id ? ' active' : ''}"
            @click="${() => {
              this.selectPlannerAction(el);
            }}"
          >
            ${el.icon
              ? html`
  ...
  getListPanel(cfg: IListVariableConfig): TemplateResult {
    ...
    if (!options.length)
      fields = html`
        <div class="text-field">${localize('instructions.no_entries_defined')}</div>
      `;
    else
