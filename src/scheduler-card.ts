...
export class SchedulerCard extends LitElement {
  ...
  private init(hass) {
    ...
    if (hass.config.unit_system && hass.config.unit_system.temperature) {
      this._config = Object.assign({ ...this._config }, <IUserConfig>{
  ...
  protected updateScheduleList(hass) {
    ...
    this.scheduleItems = Object.values(entities)
      .map((e) => ImportFromHass(e, this.Config))
      .filter((e) => e) as IScheduleEntry[];
  }
  ...
  protected render(): TemplateResult {
    if (!this.newItem && !this.editItem) {
      return html`
        <ha-card>
          ${this.getTitle()}
          <div class="card-section first">${this.getEntries()}</div>
          ${this._config.is_admin
            ? html` <div class="card-section last">
      ...
      return html`
        <ha-card>
          ...
          <div class="card-section first">
            ...
            <div class="option-list">${this.getGroups()}</div>
          </div>
          <div class="card-section">
            <div class="header">${localize('fields.entity')}</div>
            <div class="option-list">${this.getEntities()}</div>
          </div>
          ${this.getActions()}
          <div class="card-section last">
            <mwc-button outlined @click="${this._cancelEditClick}">${localize('actions.cancel')}</mwc-button>
            ${this._entry.action
              ? html`<mwc-button outlined @click="${this._confirmItemClick}">${localize('actions.next')}</mwc-button>`
              : html`<mwc-button outlined disabled>${localize('actions.next')}</mwc-button>`}
          </div>
        </ha-card>
      `;
    } else if (this._entries.length && !this._timeline) {
      return html` <ha-card> ${this.getTitle()} ${this.showEditor()} </ha-card> `;
    } else if (this._entries.length && this._timeline) {
      return html` <ha-card> ${this.getTitle()} ${this.showRoutineEditor()} </ha-card> `;
    }
    return html`
      <ha-card>
        ${this.getTitle()}
        <div class="card-section first">Something went wrong, refresh the page.</div>
      </ha-card>
    `;
  }
  ...
  getEntries(): TemplateResult[] {
    if (!this.scheduleItems || !this.scheduleItems.length)
      return [html` <div class="text-field">${localize('instructions.no_entries_defined')}</div> `];
    return this.scheduleItems.map((scheduleItem) => {
      ...
      if (scheduleItem.entries.length == 1) {
        return html`
          <div
            class="list-item${scheduleItem['enabled'] ? '' : ' disabled'}"
            @click="${() => this._editItemClick(scheduleItem.id)}"
          >
            <div class="list-item-icon">
              ${entity.icon ? html`<ha-icon icon="${PrettyPrintIcon(entity.icon)}"></ha-icon>` : ''}
            </div>
            <div class="list-item-name">${PrettyPrintName(entity.name)}</div>
            <div class="list-item-action">
              ${PrettyPrintAction(scheduleItem.entries[0], action, { temperature_unit: this._config.temperature_unit })}
            </div>
            <div class="list-item-days">${capitalize(PrettyPrintDays(scheduleItem.entries[0].days))}</div>
            <div class="list-item-time">
              ${capitalize(
                PrettyPrintTime(scheduleItem.entries[0].time, {
                  amPm: this._config.am_pm,
                  sunrise: this._config.sunrise,
                  sunset: this._config.sunset,
                }),
              )}
            </div>
            <div class="list-item-switch">
              ${scheduleItem.enabled
                ? html`<ha-switch
                    checked="checked"
                    @click="${(e) => this.toggleDisable(scheduleItem.id, e)}"
                  ></ha-switch>`
                : html`<ha-switch @click="${(e) => this.toggleDisable(scheduleItem.id, e)}"></ha-switch>`}
            </div>
          </div>
        `;
      } else {
        let description: TemplateResult[] = scheduleItem.entries.map((el) => {
          ...
          let timeStr = PrettyPrintTime(el.time, {
            amPm: this._config.am_pm,
            sunrise: this._config.sunrise,
            sunset: this._config.sunset,
          });
        ...
        return html`
          ...
          >
            ...
            <div class="list-item-name">${PrettyPrintName(entity.name)}</div>
            <div class="list-item-action">${description}</div>
            <div class="list-item-days">${capitalize(PrettyPrintDays(scheduleItem.entries[0].days))}</div>
  ...
  getGroups(): TemplateResult[] {
    ...
    return groups.map((el: IGroupElement) => {
      return html`
        <mwc-button
          class="${this._selectedGroup == el.id ? ' active' : ''}"
          @click="${() => {
            this.selectGroup(el.id);
          }}"
        >
  ...
  getEntities(): TemplateResult[] {
    ...
    return entities.map((el: IEntityElement) => {
      return html`
        <mwc-button
          class="${this._entry.entity == el.id ? ' active' : ''}"
          @click="${() => {
            this.selectEntity(el.id);
          }}"
        >
          ${el.icon ? html`<ha-icon icon="${PrettyPrintIcon(el.icon)}" class="padded-right"></ha-icon>` : ''}
          ${PrettyPrintName(el.name)}
  ...
  getActions() {
    ...
    let options_list = actions.map((el: IActionElement) => {
      return html`
        <mwc-button
          class="${this._entry.action == el.id ? ' active' : ''}"
          @click="${() => {
            this.selectAction(el.id);
          }}"
        >
          ${el.icon ? html`<ha-icon icon="${PrettyPrintIcon(el.icon)}" class="padded-right"></ha-icon>` : ''}
          ${PrettyPrintName(el.name)}
    ...
    let createTimelineButton = html`
      <div class="card-section first">
        ...
        <div class="option-list">
          <mwc-button
            class="${this._entry.action == CreateTimeline ? ' active' : ''}"
            @click="${() => {
              this.selectAction(CreateTimeline);
            }}"
          >
    ...
    return html`
      <div class="card-section">
        <div class="header">${localize('fields.action')}</div>
        <div class="option-list">${options_list}</div>
      </div>
  ...
  selectAction(action: string): void {
    ...
    if (actionCfg.variable) {
      if (actionCfg.variable.type == EVariableType.Level) {
        Object.assign(entry, <IEntry>{
          variable: {
            type: EVariableType.Level,
            value: (actionCfg.variable as ILevelVariableConfig).min,
            enabled: !(actionCfg.variable as ILevelVariableConfig).optional,
          },
        });
      } else if (actionCfg.variable.type == EVariableType.List) {
        Object.assign(entry, <IEntry>{
          variable: { type: EVariableType.List, value: (actionCfg.variable as IListVariableConfig).options[0].value },
        });
      }
    }
    this._entry = entry;
    ...
    <div class="card-section first">
     ...
     ${
       action.variable && action.variable.type == EVariableType.Level
         ? this.getLevelPanel(action.variable as ILevelVariableConfig)
         : ''
     }
     ${
       action.variable && action.variable.type == EVariableType.List
         ? this.getListPanel(action.variable as IListVariableConfig)
         : ''
     }
     ${this.getDayPicker()}
    ...
    <div class="card-section last">
      ...
      ${
        this.newItem || !this._config.is_admin
          ? ''
          : html`<mwc-button outlined @click="${this._deleteItemClick}">${localize('actions.delete')}</mwc-button>`
      }
  ...
  getDayPicker() {
    ...
    let customDayPicker =
      this._entry.days?.type == EDayType.Custom
        ? html` <div class="day-list" id="day-list-custom">
            <mwc-button
              class="day-item${customDayList.includes(1) ? ' active' : ''}"
              @click="${() => this.selectDays(1)}"
              >${localize('days_short.mon')}</mwc-button
            >
            <mwc-button
              class="day-item${customDayList.includes(2) ? ' active' : ''}"
              @click="${() => this.selectDays(2)}"
              >${localize('days_short.tue')}</mwc-button
            >
            <mwc-button
              class="day-item${customDayList.includes(3) ? ' active' : ''}"
              @click="${() => this.selectDays(3)}"
              >${localize('days_short.wed')}</mwc-button
            >
            <mwc-button
              class="day-item${customDayList.includes(4) ? ' active' : ''}"
              @click="${() => this.selectDays(4)}"
              >${localize('days_short.thu')}</mwc-button
            >
            <mwc-button
              class="day-item${customDayList.includes(5) ? ' active' : ''}"
              @click="${() => this.selectDays(5)}"
              >${localize('days_short.fri')}</mwc-button
            >
            <mwc-button
              class="day-item${customDayList.includes(6) ? ' active' : ''}"
              @click="${() => this.selectDays(6)}"
              >${localize('days_short.sat')}</mwc-button
            >
            <mwc-button
              class="day-item${customDayList.includes(7) ? ' active' : ''}"
              @click="${() => this.selectDays(7)}"
              >${localize('days_short.sun')}</mwc-button
            >
          </div>`
        : '';
    ...
    return html` <div class="card-section">
      ...
      <div class="day-list">
        <mwc-button
          class="day-item${this._entry.days?.type == EDayType.Daily ? ' active' : ''}"
          @click="${() => this.selectDays('daily')}"
          >${localize('fields.day_type_daily')}</mwc-button
        >
        <mwc-button
          class="day-item${this._entry.days?.type == EDayType.Weekdays ? ' active' : ''}"
          @click="${() => this.selectDays('weekdays')}"
          >${localize('fields.day_type_weekdays')}</mwc-button
        >
        <mwc-button
          class="day-item${this._entry.days?.type == EDayType.Custom ? ' active' : ''}"
          @click="${() => this.selectDays('custom')}"
          >${localize('fields.day_type_custom')}</mwc-button
        >
      </div>
      ${customDayPicker}
    </div>`;
  ...
  showRoutineEditor(): TemplateResult {
    ...
    return html`
      <div class="card-section first">
        ...
        <div class="summary">
          <div class="summary-entity">
            ...
            <div class="summary-text">${PrettyPrintName(entity.name)}</div>
          </div>
          <div class="summary-arrow">
            <ha-icon icon="hass:arrow-right"></ha-icon>
          </div>
          <div class="summary-action">
            ...
            <div class="summary-text">Create Timeline</div>
          </div>
        </div>
      </div>
      ${this.getDayPicker()}
      ...
      ${action && this._entry.variable && this._entry.variable.type == EVariableType.Level
        ? this.getLevelPanel(action.variable as ILevelVariableConfig, true)
        : ''}
      ${action && this._entry.variable && this._entry.variable.type == EVariableType.List
        ? this.getListPanel(action.variable as IListVariableConfig, true)
        : ''}
  ...
  getListPanel(cfg: IListVariableConfig, updateCard: boolean = false): TemplateResult {
    ...
    if (!options.length) fields = html`<div class="text-field">${localize('instructions.no_entries_defined')}</div>`;
      ...
      fields = options.map((el: IListVariableOption) => {
        return html`
          <mwc-button
            class="${this._entry.variable?.value == el.value ? ' active' : ''}"
            @click="${() => {
              this.selectListItem(el.value, updateCard);
            }}"
          >
            ${el.icon ? html`<ha-icon icon="${PrettyPrintIcon(el.icon)}" class="padded-right"></ha-icon>` : ''}
