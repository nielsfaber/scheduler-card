...
export function ArraytoDays(input: number[]): IDays {
  ...
  if (dayArray.length == alldays.length && dayArray.every((e, i) => e == alldays[i]))
    return { type: EDayType.Daily, custom_days: [] };
  else if (dayArray.length == weekdays.length && dayArray.every((e, i) => e == weekdays[i]))
    return { type: EDayType.Weekdays, custom_days: [] };
