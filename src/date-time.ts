...
export function parseTimestamp(input: string | Date): number {
  ...
  if (typeof input == "object") {
    return parseTimestamp(`${String(input.getHours()).padStart(2, '0')}:${String(input.getMinutes()).padStart(2, '0')}`);
  }
...
export function relativeTime(ts: Date, options?: { amPm: boolean }) {
  ...
  let time = `${formatTime(parseTimestamp(ts), { amPm: options?.amPm }).time}`;
  ...
  if (days_diff == 1) {
    ...
    if (ts.getHours() == 12 && ts.getMinutes() == 0) return `${localize('days.tomorrow')} ${localize('time.absolute', '{time}', localize('time.noon'))}`;
