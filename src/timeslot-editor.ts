...
export class TimeslotEditor extends LitElement {
  ...
  render() {
    return html`
      <div class="slider-container">
        <div>
          <div class="slider-track">${this.getSlots()}</div>
        </div>
      ...
      <div>
        ${this._activeEntry !== null && this.entries.length < 10
          ? html`
          ...
          : html`
            ...
            `}
        ${this._activeEntry !== null && this.entries.length > 3
          ? html`
  ...
  protected getSlots(): TemplateResult[] {
    ...
    this.entries.forEach((el, i) => {
      output.push(html`
        <div
          class="slider-slot${this._activeEntry == i ? ' active' : ''}"
      ...
      if (i < this.entries.length - 1) {
        ...
        output.push(html` <div class="slider-thumb">
          <ha-icon
            icon="hass:unfold-more-vertical"
            @mousedown="${this._handleTouchStart}"
            @touchstart="${this._handleTouchStart}"
          ></ha-icon>
          <div class="slider-thumb-tooltip" value="time" @update="${this._updateMarker}">${formatTime(ts).time}</div>
        </div>`);
      }
    });
    return output;
  ...
  getEntryAction(entry: IEntry) {
    ...
    let action = this.actions.find((e) => {
  ...
  private _handleTouchStart(e: MouseEvent | TouchEvent) {
    ...
    var mouseUpHandler = () => {
      ...
      Object.assign(entries[slotIndex + 1], <IEntry>{
        time: { value: newStop },
  ...
  static styles = css`
    ...
    div.slider-slot:hover,
    div.slider-slot.active:hover {
      opacity: 1;
    }
