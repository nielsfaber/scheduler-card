...
export class TimePicker extends LitElement {
  ...
  render() {
    return html`
      <div class="time-picker">
        ...
        ${this.getSuffix()}
        <div class="options">${this.getSunModeToggle()}</div>
      </div>
    `;
  }
  ...
  private getSunModeToggle() {
    ...
    let enabled =
      this.event || diff_sunrise <= this.maxOffset * MinutesPerHour || diff_sunset <= this.maxOffset * MinutesPerHour;
    if (enabled)
      return html`
  ...
  private getSuffix() {
    if (this.event) {
      return html`
        <div class="suffix">
          <mwc-button @click="${this.toggleBeforeAfter}"> ${this.getBeforeAfter()} </mwc-button>
      ...
      return html`
        <div class="suffix">
          <mwc-button @click="${this.toggleAmPm}"> ${this.getAmPm()} </mwc-button>
        </div>
      `;
    } else return html``;
  }
